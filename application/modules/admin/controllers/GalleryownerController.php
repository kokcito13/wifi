...
class Admin_GalleryownerController extends Zend_Controller_Action {
    ...
    public function preDispatch() {
        ...
        $this->view->headTitle()->append('Список картин');
    }

    public function indexAction() {
        $this->view->add = (object) array(
                    'link' => $this->view->url(array(), 'admin-galleryowner-add'),
                    'alt' => 'Добавить Картину',
                    'text' => 'Добавить Картину'
        );
        $this->view->breadcrumbs->add('Список картин', '');
        $this->view->headTitle()->append('Список картин');
        $this->view->galleryPhotos = Application_Model_Kernel_Galleryowner::getList(false, Application_Model_Kernel_Galleryowner::TYPE_PICTURE);
    }
    ...
    public function addAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                $this->view->galleryPhoto = new Application_Model_Kernel_Galleryowner(null, null, $this->view->idPhoto, 1, 1, 0, 0, time());
                $this->view->galleryPhoto->setContentManager($contentManager);
                $this->view->galleryPhoto->save();

                $this->_redirect($this->view->url(array(), 'admin-galleryowner-index'));
        ...
        $this->view->breadcrumbs->add('Добавить Картину', '');
